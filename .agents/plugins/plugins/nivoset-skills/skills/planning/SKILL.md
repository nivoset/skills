...
Store disposable planning state under `.tmp/planning/`, the generic repository-local root for temporary plans. Verify that `.tmp/` is excluded by the repository ignore rules before writing there, and never commit these working artifacts. If the requester explicitly asks for a durable plan, project only the accepted result into a requester-approved permanent location; do not promote checkpoints, delegation state, or provisional review artifacts by default.
